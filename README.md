# oh-my-photogrid — 와이어프레임 포토 (WireframePhoto)

## 다운로드
**[최신 APK 받기 (Releases)](https://github.com/Canine89/oh-my-photogrid/releases/latest)** — `WireframePhoto-1.2.0.apk`
...
3. 또는 PC에서 `adb install -r WireframePhoto-1.2.0.apk`

무결성 확인: `shasum -a 256 WireframePhoto-1.2.0.apk`의 결과를 릴리스에 첨부된 `.sha256` 파일과 비교한다.
...
- **디자인**: 다크룸 테마(사진이 주인공, 라임 강조색), Pretendard 서체, 스프링 모션, 떠 있는 알약형 도구 막대, 사진 색으로 번지는 캔버스 뒤 빛, 저장할 때 인화되듯 나타나는 완성본
...
- 미리보기와 같은 구도로 고해상도 JPEG/PNG를 `Pictures/WireframePhoto`에 저장 → 공유 / 배경화면으로 설정
