...
## 다운로드
**[최신 APK 받기 (Releases)](https://github.com/Canine89/oh-my-photogrid/releases/latest)** — `oh-my-photogrid-1.3.0.apk`
...
3. 또는 PC에서 `adb install -r oh-my-photogrid-1.3.0.apk`

무결성 확인: `shasum -a 256 oh-my-photogrid-1.3.0.apk`의 결과를 릴리스에 첨부된 `.sha256` 파일과 비교한다.

**이미 설치했다면(이전 이름 "와이어프레임 포토" 포함)**: 기존 앱을 지우지 말고 새 APK를 그대로 설치하면 **업데이트**로 설치된다. 설정과 저장한 사진은 그대로 남는다. 모든 버전이 같은 패키지명(`app.wireframephoto`)과 같은 서명 인증서를 쓰기 때문이다.


갤럭시 Z 폴드 8 전용 사진 콜라주 앱. 커버 화면(10:16)에서는 한 손으로 빠르게, 메인 화면(4:3)에서는 2단 구성으로 넓게 편집한다.
폴드 화면 해상도에 딱 맞는 배경화면과 SNS용 콜라주를 저장한다. 광고, 워터마크, 네트워크 권한이 없다.
...
- **디자인**: 두 가지 테마 — 다크룸(사진이 주인공, 라임 강조색)과 가족 앨범(크림 바탕, 테라코타 강조색, 홈 화면에서 전환), Pretendard 서체, 스프링 모션, 떠 있는 알약형 도구 막대, 사진 색으로 번지는 캔버스 뒤 빛, 저장할 때 인화되듯 나타나는 완성본
